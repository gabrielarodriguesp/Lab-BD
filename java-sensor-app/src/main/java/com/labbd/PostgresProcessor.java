...
public class PostgresProcessor {
    ...
    public void saveToDatabase(String jsonData) {
        try (Connection connection = dataSource.getConnection()) {
            ...
            String timestamp = jsonNode.get("timestamp").asText().replace('T', ' '); // Formatação do timestamp
            ...
            try (PreparedStatement preparedStatement = connection.prepareStatement(insertSQL)) {
                ...
                System.out.println("Rows affected: " + rowsAffected); // Log para verificar quantas linhas foram afetadas
                if (rowsAffected > 0) {
                    System.out.println("Data inserted successfully: " + jsonData);
                } else {
                    System.out.println("No rows inserted for: " + jsonData);
                }
            }
        } catch (SQLException e) {
